...
void destGameData(GameData &data)
{
	...
	{
		delete2d(data.options.stackSurface->symbmap, data.options.mainSurface->ccHeight);
...
int activeGraphic(GameData &data)
{
	...
	for (int i = 0; i < data.options.mainSurface->ccHeight; i++)
	{
		...
		{
			if ((data.options.resize || !equalG4Block(copy->symbmap[i][j], data.options.mainSurface->symbmap[i][j])))
			{
				SetConsCurPos(j, i);
